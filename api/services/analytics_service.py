...
def get_players(page: int = 1, per_page: int = 20,
    ...
    for match in matches:
        for team_key in ["team1", "team2"]:
            ...
            if team not in team_data:
                team_data[team] = {
                    "team_name": team, "matches_played": 0, "wins": 0,
                    "total_runs": 0, "seasons": set(),
                }
...
def get_team_comparison(team1: str, team2: str) -> dict[str, Any]:
    ...
    h2h_matches = [
        m for m in matches
        if (team1.lower() in m.get("team1", "").lower() or
            team1.lower() in m.get("team2", "").lower())
        and (team2.lower() in m.get("team1", "").lower() or
             team2.lower() in m.get("team2", "").lower())
    ]

    t1_wins = len([m for m in h2h_matches
                   if t1_stats and t1_stats["team_name"] == m.get("winner")])
    t2_wins = len([m for m in h2h_matches
                   if t2_stats and t2_stats["team_name"] == m.get("winner")])

    return {
        "team1": t1_stats or {"team_name": team1, "matches_played": 0, "wins": 0,
                              "losses": 0, "win_percentage": 0, "seasons_played": 0},
        "team2": t2_stats or {"team_name": team2, "matches_played": 0, "wins": 0,
                              "losses": 0, "win_percentage": 0, "seasons_played": 0},
        "head_to_head": {
...
def get_venue_stats() -> list[dict[str, Any]]:
    ...
    for venue, stats in venue_data.items():
        ...
        result.append({
            ...
            "avg_first_innings": round(sum(stats["i1_runs"]) / max(len(stats["i1_runs"]), 1), 1),
            "avg_second_innings": round(sum(stats["i2_runs"]) / max(len(stats["i2_runs"]), 1), 1),
